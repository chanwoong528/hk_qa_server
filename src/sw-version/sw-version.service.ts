import { Catch, ConflictException, HttpException, Injectable, NotFoundException, UnprocessableEntityException } from '@nestjs/common';
...
export class SwVersionService {
  ...
  async getSwVersions(swTypeId: string): Promise<SwVersion[]> {
    return await this.swVersionRepository
      .find({
        relations: ['swType', 'user'], where: { swType: { swTypeId: swTypeId } }
      });
  }

  async getSwVersionById(swVersionId: string): Promise<SwVersion> {
    return await this.swVersionRepository
      .findOne({ relations: ['swType', 'user'], where: { swVersionId: swVersionId } });
  }


  async createSwVersion(swVersion: CreateSwVersionDto, userId: string): Promise<SwVersion> {
    try {
      ...
      const targetSwType = (await this.swTypeService.getSwTypes()).find(swType => swType.swTypeId === swVersion.swTypeId);
      if (!targetSwType) {
  ...
  async updateSwVersionById(id: string, swVersion: UpdateSwVersionDto): Promise<any> {
    try {
